...
class AsyncSpimexXlsDownloader:
    @staticmethod
    async def async_download_files(link: str, session: aiohttp.ClientSession()):
        global file_counter
        try:
            response = await session.get(link)
            content = await response.read()
            async with aiofiles.open(
                f"xls_files/file_{file_counter}.xls", "wb"
            ) as file:
            ...
            logging.warning(f"Ошибка при загрузке файла: {e}")
    ...
    async def fetch_page(self, session: aiohttp.ClientSession(), page: int) -> str:
        ...
        async with session.get(url) as response:
            try:
                ...
                logging.warning(f"Ошибка в функции fetch_page: {e}")

    @staticmethod
    async def get_links(html: str) -> list | None:
...
async def parse_exel(file_path: str) -> list:
    ...
    exel_file = pd.read_excel(file_path, usecols="B:F,O", skiprows=row_number)
...
async def read_data(parsed_data: list) -> list[Trade]:
    trade_list = []
